...
const avatar = 'https://scontent.fdad3-4.fna.fbcdn.net/v/t1.6435-9/186540483_2924150637822211_8758031143615848898_n.jpg?_nc_cat=107&ccb=1-7&_nc_sid=09cbfe&_nc_ohc=PxFefvuDJtwAX_2z2ij&_nc_ht=scontent.fdad3-4.fna&oh=00_AT842javUmnXAgfY1mJYOd2VSVkzXHKKeQKKpRDbr6jvQw&oe=62E0BE21'
    ...
    const renderNavigate = useMemo(() => {
        return PROFILE_LINK.map((item, index) => {
            return <NavigateProfileItem title={item.title} key={index} to={item.to} active={pathname === item.to}>
                <Typography sx={{textDecoration: 'none', fontWeight: '600', fontSize: '.875rem'}}>{item.title}</Typography>
            </NavigateProfileItem>
        })
    }, [pathname]);
